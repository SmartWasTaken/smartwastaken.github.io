...
// --- TYPEWRITER V2.0 (A PRUEBA DE BALAS) ---
const Typewriter = ({ text, speed = 15 }) => {
  const [index, setIndex] = useState(0);

  useEffect(() => {
    setIndex(0); // Reiniciar al cambiar el texto
    
    const timer = setInterval(() => {
      setIndex((prev) => {
        if (prev < text.length) return prev + 1;
        clearInterval(timer); // Parar si llegamos al final
        return prev;
      });
    }, speed);
  ...
  // Renderizamos cortando el string original, mucho más estable que sumar
  return <span className="whitespace-pre-wrap">{text.slice(0, index)}</span>;
};
...
export function ProjectModal() {
  ...
  if (targetLocked === 'profile') {
    ...
    if (project) {
      data = { ...project, linkText: "Initialize_Protocol (Itch.io)" };
    }
  }
